# Get payment link for Cross-Border Pay (Kuajingbao)

Original name: 获取使用跨境宝支付的支付链接  
API: `com.alibaba.trade:alibaba.crossBorderPay.url.get:1` · Category: Payment  
...
|---|---|---|
| 400_4 | No order available for payment via the [Kuajingbao] payment channel | There are no orders in the order list that can be paid using Kuajingbao |
